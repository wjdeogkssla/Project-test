...
//@Component
public class ClearMyUploadFileRepo2 {
// 스케줄러 개요 실습
//	@Scheduled(cron = "*/5 * * * * * ") //5초마다 실행
//	public void clearNeedlessFiles1() {
//		
//		System.out.println("자동으로 실행되는 스케줄 메서드11111111입니다======================================");
//	}
//	
//	@Scheduled(cron = "*/10 * * * * * ") //10초마다 실행
//	public void clearNeedlessFiles2() {
//		
//		System.out.println("자동으로 실행되는 스케줄 메서드222222222입니다======================================");
//	}
	...
	public ClearMyUploadFileRepo2(MyScheduledMapper myScheduledMapper) {
	...
	private String getPathStringBeforeOneDay() {
		...
		myCalendar.add(Calendar.DAY_OF_MONTH, -2); //(오늘 날짜 -1)
...
//		return pathStringBeforeOneDay.replace("/", File.separator);
	...
	//스케줄러 메서드: @Scheduled가 명시된 메서드
	//@@Scheduled()에 명시된 시간에 자동으로 실행하는 메서드
	//- 반환타입을 void로 설정해야 됩니다.
	//- 매개변수를 갖지 않습니다.
	//@Scheduled(cron = "0 38 17 * * *") //매일 03시
	public void clearNeedlessFiles() {
		...
		if(doNotDeleteFileList == null) {
			...
			.map(attachFile -> Paths.get(attachFile.getRepoPath() + "/" +
					                     ...
					                     attachFile.getUuid() + "_" + 
					                     attachFile.getFileName()
					                     ))  //Stream<Path> 로 변환, thumbnail의 List<Path>
